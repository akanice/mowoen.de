			...
			<div class="col-md-12 col-lg-8">
				<div class="card">
					<div class="header">
						...
						<?php } ?>
						<h4 class="title"><a href="<?=base_url('admin/configs/')?>" class="btn btn-fill btn-sm btn-primary">Quay lại</a> Chọn chuyên mục sẽ hiển thị</h4>
                                ...
                                <div class="col-sm-10">
                                    <select data-placeholder="Chọn chuyên mục hiển thị..." class="chosen-select form-control" multiple style="width:100%;" tabindex="4" name="cat_available[]">
                                        <?php foreach($categories as $a){?>
                                            <option value="<?=$a->id?>" <?php if (($cat_available != '') and ($cat_available)) { if(in_array($a->id,$cat_available)) { echo 'selected'; }}?>><?=$a->title?></option>
                                        <?php }?>
                                    </select>
                                </div>
							</div>
