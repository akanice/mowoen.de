...
class NewsCategoryModel extends MY_Model {
	...
	public function get_categories($type,$title,$limit,$offset) {
		...
		if ($type) {
			$this->db->where('news_category.type',$type);
		}
		if ($title) {
	...
	protected function _nForLoop($type='other',$post_type='post',$data, $parent = "0", $level = 1) {
        foreach ($data as $key => $value) {
            if ($value["parent_id"] == $parent) {
                ...
                // next loop
                $this->_nForLoop($type,$post_type,$data, $value["id"], $level + 1);
            }
        }
    }

    public function getSortedCategories($type='other',$post_type='post') {
		$this->_nForLoop($type,$post_type,$this->db->get_where('news_category',array('type'=>$type,'post_type'=>$post_type))->result_array());
